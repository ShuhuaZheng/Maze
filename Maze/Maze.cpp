...
void Maze::setUpMaze() {
  ...
  // Set up the bottom wall of the last row
    for (int j = last-1; j>=last-myWidth+1; --j)
...
bool Maze::isConnectedNeighbor(int room_i, int room_j)
{
 ...
 void Maze::getCellWallValue(int cellInd, int & topVal, int & botVal, int & leftVal, int & rightVal)
 {
    topVal = mySets.getWallVal(cellInd, 0);
    botVal = mySets.getWallVal(cellInd,1);
    rightVal = mySets.getWallVal(cellInd,2);
    leftVal = mySets.getWallVal(cellInd,3);
 }